...
import { renderSSRContextScript, getSSRStoreScript } from '/@/universal'
...
const renderHTML = async (vueApp: VueApp, url: string, error?: any) => {
  ...
  // 8. SSR context
  // const STORE_SCRIPT = getSSRStoreScript(serialize(store.state.value))
  // // TODO: SSR context?
  // const SSR_CONTEXT_SCRIPT = renderSSRContextScript(
  //   serialize({
  //     url,
...
export const render: UniversalServerRenderer = async (request: IncomingMessage) => {
  ...
  const app = createVueApp({
    ...
    theme: (Cookies.get(THEME_STORAGE_KEY) as Theme) || Theme.Default
  })
